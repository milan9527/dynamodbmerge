...
public class DynamoDBSink extends RichSinkFunction<GameMailboxMergedItem> {
    ...
    public void invoke(GameMailboxMergedItem value, Context context) {
        try {
            ...
            LOG.error("Error writing/deleting item in DynamoDB: {}", e.getMessage(), e);
        }
    }
    
    /**
     * Delete an item from the target table
     */
    private void deleteItem(GameMailboxMergedItem value) {
        try {
            // Create a key map with the partition key
            Map<String, AttributeValue> key = new HashMap<>();
            key.put("PK", AttributeValue.builder().s(value.getPK()).build());
            ...
            LOG.info("Successfully deleted item with PK: {} from table: {}", value.getPK(), tableName);
        } catch (Exception e) {
            LOG.error("Error deleting item from DynamoDB: {}", e.getMessage(), e);
        }
    }
